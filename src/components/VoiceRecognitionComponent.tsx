...
const VoiceRecognitionComponent: React.FC<VoiceRecognitionComponentProps> = ({
  ...
  const [state, setState] = useState<
    "listening" | "interpreting" | "reviewing"
  >("listening");
  ...
  const handleOperationTypeChange = (
    ...
    setOperations((prev) =>
      prev.map((op, i) => (i === index ? { ...op, type: value } : op)),
    );
  };
  ...
  const handleItemNameChange = (index: number, value: string | null) => {
    if (value !== null) {
      setOperations((prev) =>
        prev.map((op, i) =>
          i === index ? { ...op, item: { ...op.item, name: value } } : op,
        ),
      );
    }
  };
  ...
  const handleSave = async (updatedItem: Operation["item"]): Promise<void> => {
    setOperations((prev) =>
      prev.map((op) =>
        op === editingItem ? { ...op, item: updatedItem } : op,
      ),
    );
...
const handleConfirm = async () => {
  ...
  const TableRow: React.FC<TableRowProps> = ({
    ...
    const itemExists = databaseItems.some(
      (item) => item.name.toLowerCase() === comboboxValue.toLowerCase(),
    );
      ...
      <Table.Tr>
        ...
        <Table.Td>
          ...
          >
            ...
            <Combobox.Dropdown>
              <Combobox.Options>
                {filterSimilarItems(databaseItems, comboboxValue).map(
                  (item) => (
        ...
        <Table.Td>
          {op.item.categories.map(
            (category: { name: string; color: string }, catIndex: number) => (
              <Badge
                key={catIndex}
